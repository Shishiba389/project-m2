...
import {
  ...
  PanelLeft, PanelRightClose, Plus, Redo2, RotateCcw, Search, Settings, Sparkles,
...
export function MinimaWorkspace() {
  ...
  return <main className="minima-app">
    <header className="topbar"><div className="topbar-left"><span className="window-controls" aria-hidden="true"><i /><i /><i /></span><Button variant="ghost" size="icon" aria-label="Toggle sidebar"><PanelLeft /></Button><Button variant="ghost" size="sm" className="back-button" aria-label="Back"><ArrowLeft /> Back</Button><Button variant="ghost" size="icon" aria-label="Undo"><Undo2 /></Button><Button variant="ghost" size="icon" aria-label="Redo"><Redo2 /></Button></div><div className="document-name">{view === "editor" || view === "compare" ? active.name : section === "settings" ? "App Settings" : "MINIMA Resize"}</div><div className="topbar-actions"><Minus size={14} /><span>100%</span><Plus size={14} /><Button variant="ghost" size="icon" aria-label="Help"><CircleHelp /></Button></div></header>
    <aside className="rail" aria-label="Main navigation"><div className="rail-main">{navItems.map(({ id, label, icon: Icon }) => <button key={id} className={section === id ? "active" : ""} aria-current={section === id ? "page" : undefined} onClick={() => { if (id === "export") { setSection("images"); setExportOpen(true); return; } setSection(id); if (id === "images") setView("gallery"); }}><Icon aria-hidden="true" /><span>{label}</span></button>)}</div><div className="rail-bottom"><button onClick={() => setExportOpen(true)}><Upload aria-hidden="true" /><span>Export</span></button><button className={section === "settings" ? "active" : ""} aria-current={section === "settings" ? "page" : undefined} onClick={() => setSection("settings")}><Settings aria-hidden="true" /><span>Settings</span></button></div></aside>
    <section className="workspace">
      {section === "import" && <><ImportScreen onBrowse={() => fileInput.current?.click()} onFiles={importFiles} /><IdleInspector /></>}
    ...
    {section === "import" && <footer className="statusbar"><span>0 images loaded</span><div className="view-switch" role="group" aria-label="Workspace view"><button onClick={() => { setSection("images"); setView("gallery"); }}>Gallery</button><button onClick={() => { setSection("images"); setView("editor"); }}>Editor</button></div><span /></footer>}
    {(section === "images" || section === "export") && <footer className="statusbar"><span>{selected.length} selected</span><div className="status-actions"><Button variant="ghost" size="sm" onClick={() => setView("compare")}><Columns2 /> Compare</Button><Button variant="ghost" size="sm" onClick={() => setView("review")}><TriangleAlert /> Review</Button><div className="view-switch" role="group" aria-label="Workspace view"><button className={view === "gallery" ? "active" : ""} onClick={() => setView("gallery")}>Gallery</button><button className={view === "editor" ? "active" : ""} onClick={() => setView("editor")}>Editor</button></div><ExportDialog open={exportOpen} onOpenChange={setExportOpen} assets={assets} /></div><span>1801 × 2600 px · RGB · PNG · 2.4 MB</span></footer>}
...
function ImportScreen({ onBrowse, onFiles }: { onBrowse: () => void; onFiles: (files: FileList) => void }) {
  ...
  return <div className="empty-workspace" onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(e) => { e.preventDefault(); setDragging(false); onFiles(e.dataTransfer.files); }}><button className={`drop-zone ${dragging ? "dragging" : ""}`} onClick={onBrowse}><span className="drop-icon"><FolderOpen aria-hidden="true" /><Upload aria-hidden="true" /></span><span>Drop images here&nbsp; or</span><strong>Browse (PNG, JPG, WEBP, TIFF)</strong></button></div>;
}
